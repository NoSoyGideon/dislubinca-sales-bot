...
class ExcelService(BaseExcelService):
    _lock_global_excel = threading.RLock()
    _lock_owner_thread = None
    _lock_depth = 0

    @classmethod
    def esta_ocupado(cls) -> bool:
        """Indica si otra hebra está usando el Excel global del bot."""
        thread_id = threading.get_ident()
        return cls._lock_owner_thread is not None and cls._lock_owner_thread != thread_id

    @classmethod
    def _adquirir_lock_global(cls):
        """Adquiere un lock reentrante por proceso para serializar escrituras/lecturas de Excel."""
        thread_id = threading.get_ident()
        if cls._lock_owner_thread == thread_id:
            cls._lock_depth += 1
            return

        cls._lock_global_excel.acquire()
        cls._lock_owner_thread = thread_id
        cls._lock_depth = 1

    @classmethod
    def _liberar_lock_global(cls):
        """Libera el lock solo cuando el hilo que lo adquirió termina su pila completa."""
        thread_id = threading.get_ident()
        if cls._lock_owner_thread != thread_id:
            return

        cls._lock_depth -= 1
        if cls._lock_depth <= 0:
            cls._lock_owner_thread = None
            cls._lock_depth = 0
            cls._lock_global_excel.release()

    @classmethod
    @contextmanager
    def _bloquear_excel_global(cls):
        """Context manager que espera a que termine otro usuario/hilo y permite reentrada del mismo hilo."""
        cls._adquirir_lock_global()
        try:
            yield
        finally:
            cls._liberar_lock_global()
    ...
    def _garantizar_archivo_y_pestana(self, nombre_archivo: str, plantilla_base_nombre: str, pestana_target: str, ruta_local: str, fecha_str: str = None) -> openpyxl.Workbook:
        ...
        """
        with self._bloquear_excel_global():
    ...
    def actualizar_cobranza_acumulada_excel(self, lote_cobros: dict, fecha_str: str) -> bool:
        """
        [FUNCIÓN AUXILIAR DE RELEVO DE COBRANZA - AUDITADA]
        ...
        """
        print(f"🔍 [AUDITORÍA COBRANZA 1] Entrando a 'actualizar_cobranza_acumulada_excel' con fecha: {fecha_str}...")
        print(f"🔍 [AUDITORÍA COBRANZA 1.1] Lote de cobros recibido: {lote_cobros}")

        t_inicio = time.time()

        with self._bloquear_excel_global():
            print("🔍 [AUDITORÍA COBRANZA 2] Candado global '_bloquear_excel_global' ADQUIRIDO.")
            
            try:
                dt = datetime.strptime(fecha_str, "%Y-%m-%d")
                dia_semana = dt.weekday()
                print(f"🔍 [AUDITORÍA COBRANZA 2.1] Día de la semana calculado: {dia_semana} (0=Lunes, 4=Viernes, 5=Sábado, 6=Domingo)")

                fecha_siguiente = self._calcular_fecha_siguiente(fecha_str)
                print(f"🔍 [AUDITORÍA COBRANZA 2.2] Fecha siguiente calculada: {fecha_siguiente}")
                
                if dia_semana >= 5:
                    print(f"ℹ️ [ExcelService] Fecha {fecha_str} es fin de semana (Día {dia_semana}). Se omite el relevo de cobranza.")
                    return True

                mapa_columnas_dias = ContactoMatutinoMap.COLUMNAS_DIAS_COBRANZA
                col_dia = mapa_columnas_dias.get(dia_semana)
                print(f"🔍 [AUDITORÍA COBRANZA 2.3] Columna de día obtenida de ContactoMatutinoMap: '{col_dia}'")

                if not col_dia:
                    print(f"❌ [AUDITORÍA COBRANZA ERROR] No se encontró mapeo de columna para el día de semana: {dia_semana}")
                    return False
                ...
                print(f"🔍 [AUDITORÍA COBRANZA 3] Nombres calculados:")
                print(f"   - Archivo Matutino: {archivo_matutino}")
                print(f"   - Pestaña Target: {pestana_target}")
                print(f"   - Ruta Local: {ruta_local}")
                print(f"   - Ruta Dropbox: {ruta_dropbox}")

                print(f"🔍 [AUDITORÍA COBRANZA 4] Invocando '_garantizar_archivo_y_pestana'...")
                wb = self._garantizar_archivo_y_pestana(
                    nombre_archivo=archivo_matutino,
                    plantilla_base_nombre="CONTACTO_MATUTINO_BASE.xlsm",
                    pestana_target=pestana_target,
                ...
                print(f"🔍 [AUDITORÍA COBRANZA 4.1] Workbook obtenido con éxito.")

                if pestana_target not in wb.sheetnames:
                    print(f"❌ [AUDITORÍA COBRANZA ERROR] La pestaña '{pestana_target}' no existe en el Workbook. Pestañas disponibles: {wb.sheetnames}")
                    return False

                ws = wb[pestana_target]
                print(f"💵 [ExcelService] Relevando cobranza para el día {ws.title} en Columna {col_dia}...")

                # Bucle de inyección de datos
                for ruta_id, monto in lote_cobros.items():
                    print(f"🔍 [AUDITORÍA COBRANZA 5] Procesando Ruta ID: {ruta_id} | Monto: {monto}")
                    
                    if monto is None or float(monto) <= 0:
                        print(f"  ⏩ [AUDITORÍA COBRANZA 5.1] Monto nulo o cero para Ruta {ruta_id}. Se omite.")
                        continue

                    encontrado = False
                    print(f"🔍 [AUDITORÍA COBRANZA 5.2] Escaneando rango P9:P20 en busca de la ruta '{ruta_id}'...")
                    for fila in range(ContactoMatutinoMap.FILA_INICIO_RUTAS, ContactoMatutinoMap.FILA_FIN_RUTAS+1):
                        val_celda = ws[f"P{fila}"].value
                        # Print detallado celda por celda para verificar discrepancias de tipo (str vs int)
                        print(f"    - Revisando Celda P{fila} -> Valor actual: '{val_celda}' (Tipo: {type(val_celda).__name__})")
                        
                        if val_celda is not None and str(ruta_id).strip() == str(val_celda).strip():
                            monto_float = float(monto)
                            ws[f"{col_dia}{fila}"] = monto_float
                            print(f"  🎯 Ruta {ruta_id} encontrada en P{fila} ➔ Cobranza de ${monto_float} asignada en {col_dia}{fila}")
                            encontrado = True
                            break
                ...
                print(f"🔍 [AUDITORÍA COBRANZA 6] Guardando cambios en el archivo local: {ruta_local}...")
                wb.save(ruta_local)
                print(f"🔍 [AUDITORÍA COBRANZA 6.1] Archivo guardado localmente. Cerrando Workbook...")
                wb.close()
                
                print(f"🔍 [AUDITORÍA COBRANZA 7] Aplicando pausa táctica (time.sleep(1.5)) para liberar bloqueo de archivo OS...")
                time.sleep(1.5)

                print(f"⚠️ [AUDITORÍA COBRANZA 8] DISPARANDO SUBIDA A DROPBOX hacia: {ruta_dropbox}...")
                t_dropbox_start = time.time()
                
                exito_subida = self.dropbox.subir_archivo(ruta_local, ruta_dropbox)
                
                print(f"🔍 [AUDITORÍA COBRANZA 8.1] Respuesta de Dropbox obtenida en {round(time.time() - t_dropbox_start, 2)}s. Resultado: {exito_subida}")
                
                print(f"🎉 [AUDITORÍA COBRANZA FIN] Proceso finalizado con éxito en {round(time.time() - t_inicio, 2)}s.")
                return exito_subida

            except Exception as e:
                print(f"❌ [ExcelService] Error crítico en relevo de cobranza acumulada: {e}")
                import traceback
                print(f"💥 [TRACEBACK COMPLETO]:\n{traceback.format_exc()}")
                return False

            finally:
                print("🔍 [AUDITORÍA COBRANZA FINALLY] Entrando al bloque de limpieza...")
                if os.path.exists(ruta_local):
                    try:
                        os.remove(ruta_local)
                        print(f"🧹 [AUDITORÍA COBRANZA FINALLY] Archivo temporal eliminado correctamente: {ruta_local}")
                    except Exception as err_rm:
                        print(f"⚠️ [AUDITORÍA COBRANZA FINALLY] No se pudo borrar el temporal: {err_rm}")
                else:
                    print("🧹 [AUDITORÍA COBRANZA FINALLY] No se requirió eliminar temporal (no existe).")
                
                print("🔍 [AUDITORÍA COBRANZA FINALLY] Liberando '_bloquear_excel_global'...")
    # ========================================================
        ...
        with self._bloquear_excel_global():
            ...
            try:
                ...
                if pestana_control:
                    ...
                    for fila in range(ContactoMatutinoMap.FILA_INICIO_CONTROL, ContactoMatutinoMap.FILA_FIN_CONTROL + 1):
